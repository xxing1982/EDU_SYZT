ctrls.controller("schoolAdminCtrl",['$scope', '$rootScope', 'SchoolAdmin', 'Pageable', 'GetSchoolAdmin', '$stateParams',
	function($scope, $rootScope, SchoolAdmin, Pageable, GetSchoolAdmin, $stateParams){
		$scope.selectSchool = {
			isEdit: true,
			isShowSchool: true,
		...
		$scope.AddSys = function(){
			SchoolAdmin.Add($scope.campusidSelected, $scope.add.userid, $scope.add.password, function(){
				$("#addModal").modal('hide');
			})
		}
		...
		$scope.EditSys = function(){
			...
			SchoolAdmin.Update(item.userId, item.oldpassword, item.newpassword, function(){
		...
		$scope.deleteSys = function(item){
			$rootScope.confirm_modal = {};
				...
				$rootScope.confirm_modal.click = function(){
					SchoolAdmin.Delete(item.id, function(){
						location.reload();
					})
				}
				$('#confirm-modal').modal();
		...
		$scope.createPageable = function (){
        	...
        	$scope.searchPageable.build(GetSchoolAdmin);
