...
public interface SpeedTestRecordRepository extends JpaRepository<SpeedTestRecord,Long> {
	...
	//select * from (select * from `test` order by `date` desc) `temp`  group by category_id order by `date` desc
	 ...
	 @Query(value="select * from (select * from (select * from speed_test_record where school_id = :schoolId) as a group by user_id order by speed desc, score desc ) as b order by speed desc,score desc limit :num",nativeQuery=true)
	 List<SpeedTestRecord> getTopStudentBySchool(@Param("num") int num,@Param("schoolId") long schoolId);
	 
	 @Query(value="select * from (select * from speed_test_record group by user_id order by speed desc, score desc ) as a order by speed desc,score desc limit :num",nativeQuery=true)
