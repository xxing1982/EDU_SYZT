...
//    ClazzSumStatistic getSumStatisticClazz(@Param("grade") int grade);
    ...
    @Query("select c from Clazz c where c.grade = :grade and c.campus.id = :campusId")
    List<Clazz> findByGradeAndCampus( @Param("grade")int grade, @Param("campusId")long campusId );
